...
/**
 * RocketMQ 核心架构：NameServer → Broker(Master+Slave) → Topic → MessageQueue → Producer → Consumer
 *
 * <p>手写简化 NameServer（路由注册表 Map）、Topic → MessageQueue 映射（写队列/读队列数量）、
 * 消息存储模型 CommitLog + ConsumeQueue（文件追加 + 索引定位）。
 *
 * <p>核心组件关系：
 * <pre>
 * NameServer（路由中心）
 *   ├── Topic → List&lt;QueueData&gt;（队列元数据）
 *   │     ├── BrokerName
 *   │     ├── ReadQueueNums / WriteQueueNums
 *   │     └── Perm（读写权限 6=读写 4=只读）
 *   └── BrokerAddrTable（Broker 地址映射）
 *
 * Broker（消息中转）
 *   ├── CommitLog（所有消息物理存储，顺序写）
 *   │     ├── 每个文件 1GB = 1024 * 1024 * 1024
 *   │     └── 文件名 = 起始偏移量（20位0填充）
 *   └── ConsumeQueue（逻辑队列索引）
 *         └── 每条 20 字节：offset(8) + size(4) + tagCode(8)
 *
 * Producer（生产者）→ 通过 NameServer 发现 Broker → 选择 MessageQueue → 发送
 * Consumer（消费者）→ 通过 NameServer 发现 Broker → 拉取消息 → ACK
 * </pre>
 *
 * @author study-tuling
 */
